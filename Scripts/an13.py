...
PARR_micromol_kg_day=PARR_nmol_l_h/1000*24/(dens_PARR/1000)
...
for i_isop in range(0,reference_isopycnal_list.size):
    ...
    for i in range(0,doxy.shape[0]):
        ...
        # Here, for the i-th profile, I select the PARR, density and depth profiles of Ecopart data, excluding the nan values
...
ax[0].plot(slopes_list*-1, depth_isopycnal, 'k')#, linestyle='dashed')
ax[0].scatter(slopes_list*-1, depth_isopycnal, c='black',s=5)
ax[0].fill_betweenx(depth_isopycnal,slopes_ci_list[:,1]*-1,slopes_ci_list[:,0]*-1,facecolor='b',color='gray',alpha=0.5,label='O$_2$')
...
ax[1].scatter(R2_list[R_list<0], depth_isopycnal[R_list<0], c='r',s=5)#, linestyle='dashed')
ax[1].set_xlim(0,1.3)
...
fig.savefig('../Plots/an13/01_OxygenRespirationRate_vs_depth_deltarho_%0.2f_an13.pdf' % delta_rho, dpi=200)
plt.close()
