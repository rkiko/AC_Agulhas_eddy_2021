...
for i in range(0, pres0.shape[0]):
    ...
    # I do the plot only if the profile sampled below 1500m
    y = pres0[i, :]
    sel = (~np.isnan(y)) & (y != 99999)
    y = y[sel]
    if y.max()<1500:    continue
    ...
    ax = fig.add_axes([0.25, 0.15, width, height], ylim=(1000, 2000), xlim=(0, 23))
    ...
    ax = fig.add_axes([0.25, 0.15, width, height], ylim=(1000, 2000), xlim=(34, 36))
    plt.scatter(x, y, c='b', s=0.1,zorder=10)
    plt.plot(x, y, 'b', linewidth=0.25,zorder=11)
    ...
    plt.savefig('%s/TEMPvsPRES/TempVsPres_Profile%03d_%05.2fdegreee%02ddays_an49.pdf' % (plotdir,int(i+1),dist_thr,time_lag), dpi=200)
    plt.close()
    ...
    int(i + 1), date_time_obj.day, date_time_obj.month, date_time_obj.year, date_time_obj.hour, date_time_obj.minute,
    inoutEddy,n_profiles_ext,dist_thr*111,time_lag), fontsize=10)
    plt.savefig('%s/PSALvsPRES/PsalVsPres_Profile%03d_%05.2fdegreee%02ddays_an49.pdf' % (plotdir,int(i+1),dist_thr,time_lag), dpi=200)
    plt.close()
