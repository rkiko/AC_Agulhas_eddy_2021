...
#######################################################################
# Delayed mode test for JULD
# I check that JULD is in cronological order
#######################################################################
plt.plot(Date_Num,'b.')
plt.close()
a=Date_Num[1:-1]-Date_Num[0:-2]
if a.size==sum(a>=0):
    print('Juld is in cronological order: delayed mode test ok')

#######################################################################
# Delayed mode test for lon and lat
# I check that longitude and lat do not have outliers
#######################################################################
plt.scatter(lon,lat,c='b')
plt.plot(lon,lat,'b')
plt.close()

Q1lon = np.quantile(lon,0.25)
Q3lon = np.quantile(lon,0.75)
Q1lat = np.quantile(lat,0.25)
Q3lat = np.quantile(lat,0.75)
IQR_lon = Q3lon - Q1lon
IQR_lat = Q3lat - Q1lat

if sum((lon < (Q1lon - 1.5 * IQR_lon)) |(lon > (Q3lon + 1.5 * IQR_lon)))==0:
    print('Longitude has no outliers, delayed mode test ok!')
else:
    print('Longitude does have outliers, delayed mode test NOT ok')

if sum((lat < (Q1lat - 1.5 * IQR_lat)) |(lat > (Q3lat + 1.5 * IQR_lat)))==0:
    print('Latitude has no outliers, delayed mode test ok!')
else:
    print('Latitude does have outliers, delayed mode test NOT ok')

#######################################################################
# Delayed mode test for pressure
# I plot TEMP vs. PRES, and PSAL vs. PRES for each profile to check whether there are no errors
#######################################################################
...
for i in range (0,pres.shape[0]):
    ...
    ax = fig.add_axes([0.25, 0.2, width, height], ylim=(0, 1000), xlim=(0, 23))
    ...
    plt.savefig('../Plots/an47/TEMPvsPRES/TempVsPres_Profile%03d_an47.pdf' % int(i+1), dpi=200)
    plt.close()
...
for i in range(0, pres.shape[0]):
    ...
    x=x[sel];y=y[sel]
    fig = plt.figure(1, figsize=(3, 3))
    ax = fig.add_axes([0.25, 0.2, width, height], ylim=(0, 1000), xlim=(34, 36))
    plt.scatter(x,y,c='b',s=0.1)
    plt.plot(x,y,'b',linewidth=0.1)
