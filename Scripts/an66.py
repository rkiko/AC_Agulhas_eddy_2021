...
np_tmp=temp.shape[0]-radius_Vmax_4float.size
...
# a_file = open("%s/an66/data_an66.pkl" % storedir, "wb")
# pickle.dump(dictionary_data, a_file)
# a_file.close()

list_dates=list_dates[sel_insideEddy]
sel_insideEddy=np.append(sel_insideEddy,[False for i in range(0,np_tmp)])
...
for ipar in range(0,parameter_ylabel_list.__len__()):
    ...
    plt.savefig('../Plots/an66/TimeSeries%02dA%s_an66.pdf' % (ipar,parameter_shortname_list[ipar]),dpi=200)
    plt.close()
    ...
    # I add the grid
    plt.grid(color='k', linestyle='dashed', linewidth=0.5)
    plt.savefig('../Plots/an66/TimeSeries%02dB%s_an66.pdf' % (ipar,parameter_shortname_list[ipar]),dpi=200)
    plt.close()
...
plt.grid(color='k', linestyle='dashed', linewidth=0.5)
plt.savefig('../Plots/an66/0_200m_MiP_MaP_bbpPOC_vs_time_an66.pdf' ,dpi=200)
plt.close()
...
plt.grid(color='k', linestyle='dashed', linewidth=0.5)
plt.savefig('../Plots/an66/200_600m_MiP_MaP_bbpPOC_vs_time_an66.pdf' ,dpi=200)
plt.close()
...
plt.ylim(0,np.max(bbp_POC_mld)*1.05)
...
plt.legend()
# I save
plt.savefig('../Plots/an66/ZTimeSeries_bbp_ML_an66.pdf' ,dpi=200)
plt.close()
...
plt.ylim(0,np.max(MaP_POC_mld)*1.05)
plt.vlines(day_start_eddy_merging, ymin=0, ymax=600, color='k')
plt.vlines(day_end_eddy_merging, ymin=0, ymax=600, color='k')
# I set xticks
nxticks = 10
...
plt.legend()
# I save
plt.savefig('../Plots/an66/ZTimeSeries_MaP_ML_an66.pdf' ,dpi=200)
plt.close()
...
#
# a_file = open("%s/an66/data_an66.pkl" % storedir, "rb")
# data_an66 = pickle.load(a_file)
# bbp_POC = data_an66['bbp_POC']
# sel_insideEddy = data_an66['sel_insideEddy']
# Date_Num_bbp = data_an66['Date_Num_bbp']
# Date_Vec_bbp = data_an66['Date_Vec_bbp']
# Date_Num_bbp_calendar = data_an66['Date_Num_bbp_calendar']
# depth_bbp = data_an66['depth_bbp']
# a_file.close()
#
# dictionary_data = {"bbp_POC": bbp_POC,"sel_insideEddy": sel_insideEddy,"Date_Num_bbp": Date_Num_bbp,
#                    "Date_Vec_bbp": Date_Vec,"depth_bbp": depth_bbp,"Date_Num_bbp_calendar": Date_Num_bbp_calendar,
