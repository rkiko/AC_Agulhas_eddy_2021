...
for idx in range(0, len(list_indexes)):
    ...
    for threshold_value in threshold_value_list:
        ...
        plot3 = plt.scatter(Date_Num_threshold, depth_threshold, c='white')
        plot4= plt.plot(np.linspace(Date_Num_limit[0],Date_Num_limit[1],20),np.linspace(Date_Num_limit[0]*interpol.slope/86400+interpol.intercept,Date_Num_limit[1]*interpol.slope/86400+interpol.intercept,20),c='white')
...
plt.plot(sizeclass_list,np.abs(sink_k2002_2),label='Kriest et al., 2002, Tab.2 (8)')
plt.plot(sizeclass_list,np.abs(sink_k2002_1),label='Kriest et al., 2002, Tab.2 (9)')
