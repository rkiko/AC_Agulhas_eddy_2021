...
for i in range(0,lon.size):
    daytmp=date.toordinal(date(Date_Vec[i,0], Date_Vec[i,1], Date_Vec[i,2]))+366
...
for ipar in range(0,parameter_ylabel_list.__len__()):
    ...
    plot2=ax.contourf(x_parameter,y1_parameter, parameter_interp_depth)
    ax.invert_yaxis()
    ...
    ax2.plot(Date_Num_Eddy - date_reference_datenum, radius_Vmax, color_distance)
    ax2.scatter(Date_Num, dist_km, s=20, marker='*', c=color_distance)
    ...
    # I save
    plt.savefig('../Plots/an14/TimeSeries_%s_vs_01depth_an14.pdf' % parameter_shortname_list[ipar],dpi=200)
    plt.close()
    ...
    ax.set_xlim(Date_Num.min(), Date_Num.max())
    ax.set_ylim(set_ylim_lower, set_ylim_upper)
    plot2 = ax.contourf(x_parameter,y2_parameter, parameter_interp_dens)
    ax.invert_yaxis()
    # plt.title('%smm' % NP_sizeclass, fontsize=18)
    # I set xticks
    nxticks = 10
    ...
    ax.set_xticklabels(xticklabels)
    plt.xticks(rotation=90, fontsize=12)
    ax.set_ylabel('Density (kg/m$^3$)', fontsize=18)
    # I add the grid
    plt.grid(color='k', linestyle='dashed', linewidth=0.5)
    # I add the radius and distance
    ax2 = ax.twinx()
    ...
    cbar.ax.set_xlabel(parameter_ylabel_list[ipar], fontsize=18)
    # I save
    plt.savefig('../Plots/an14/TimeSeries_%s_vs_02dens_an14.pdf' % parameter_shortname_list[ipar],dpi=200)
    plt.close()
