...
if __name__ == '__main__':
    ...
    df = calculate_nbss_slope_func(df)
    df = calculate_flux_func(df)
    df = calculate_mip_and_map_abundance_func(df)
    df = calculate_mip_and_map_biovolume_func(df)
    df = df[["Cruise", "RAWfilename", "CTD_filename", "Latitude", "Longitude", "Date_Time", "Pressure [dbar]", "Vol [L] (sampled for this depth bin)", "MiP_abun", "MaP_abun", "MiP_biov", "MaP_biov", "NBSS slope", 'NBSS intercept', "NBSS slope stde", "Flux_mgC_m2"]]
    if test == 1:
        df.to_csv('Ecopart_mip_map_nbss_flux_data_test.tsv', sep='\t', index=False)
    else:
        df.to_csv('Ecopart_mip_map_nbss_flux_data.tsv', sep='\t', index=False)
