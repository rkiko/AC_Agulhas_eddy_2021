...
#endregion
#region I save some values for latex
...
# plt.savefig('../Plots/Supplementary/Anom_ConsTemp_v08.pdf',dpi=200)
# plt.close()
...
plt.grid(color='k', linestyle='dashed', linewidth=0.5)
plt.savefig('../Plots/Fig_Main_v08/Supplementary/Supplementary_SatChl_v08.pdf' ,dpi=200)
plt.close()
#endregion
...
from scipy.interpolate import griddata
#endregion
#region processing and anaylsing
#Here I define the time at which I want to finish the time series in the plot
day_end_timeseries=np.array([2021,9,24])
day_end_timeseries=matlab_datenum(day_end_timeseries)
...
for ipar in range(0,parameter_ylabel_list.__len__()):
    ...
    plt.grid(color='k', linestyle='dashed', linewidth=0.5)
    # I save
    plt.savefig('../Plots/Fig_Main_v08/Supplementary/TimeSeries_ML/TimeSeries_ML_%s_v08.pdf' % parameter_shortname_list[ipar], dpi=200)
    plt.close()
    ...
    if ipar == 1:
        ...
        plt.grid(color='k', linestyle='dashed', linewidth=0.5)
        # I save
        plt.savefig('../Plots/Fig_Main_v08/Supplementary/TimeSeries_ML/TimeSeries_ML_chla_integrated_v08.pdf', dpi=200)
        plt.close()
    ...
    plt.grid(color='k', linestyle='dashed', linewidth=0.5)
    # I save
    plt.savefig('../Plots/Fig_Main_v08/Supplementary/TimeSeries_EddyCore/TimeSeries_EddyCore_%s_v08.pdf' % parameter_shortname_list[ipar], dpi=200)
    plt.close()
#endregion
#region I save the some values for the latex document
#######################################################################
...
sel_insideEddy = data_dist_radius['sel_insideEddy']
datenum_profiles = data_dist_radius['Datenum']
if (sel_insideEddy.size<Date_Num_bbp.size):
    npoints_lacking = Date_Num_bbp.size - sel_insideEddy.size
...
if list_dates.size>sel_insideEddy.size:
...
for i_depth in range(0,list_depths_plot.size-1):
    ...
    plt.grid(color='k', linestyle='dashed', linewidth=0.5)
    plt.savefig('../Plots/Fig_Main_v08/Supplementary/ExtendedPSD/ExtendedPSD_%dm_v08.pdf' % (depth_tmp), dpi=200)
    plt.close()
    ##############
    #POC plot normalised
    ##############
    ...
    plt.title('Depth: %d m' % depth_tmp, fontsize=10)#; R$^2$=%0.2f, Signif: %s' % (depth_tmp, interpol.rvalue ** 2, signif_label)
    plt.grid(color='k', linestyle='dashed', linewidth=0.5)
    plt.savefig('../Plots/Fig_Main_v08/Supplementary/ExtendedPSD_Norm/ExtendedPSD_Norm_%dm_v08.pdf' % (depth_tmp), dpi=200)
    plt.close()
#endregion
...
########################################################################################################################
########################################################################################################################
#These two figures are now useless as they are identical to those plotted in Fig_Main_v08 called Fig04A_Koestner_v08 and Fig04B_Koestner_v08
...
#region I loop on the different depths
#######################################################################
...
#I comment these two figures as they are identical to those plotted in Fig_Main_v08 called Fig04A_Koestner_v08 and Fig04B_Koestner_v08
...
# plt.grid(color='k', linestyle='dashed', linewidth=0.5)
# plt.savefig('../Plots/Fig_Main_v08/Supplementary/BulkOxyPARR_a_v08.pdf' ,dpi=200)
# plt.close()
#
#
# ########################################################################################################################
...
# plt.grid(color='k', linestyle='dashed', linewidth=0.5)
# plt.savefig('../Plots/Fig_Main_v08/Supplementary/BulkOxyPARR_b_v08.pdf' ,dpi=200)
# plt.close()
#endregion
#endregion
