...
### I load eddy tajectories updated by Remi on 16 06 2022
filename_traj_eddy1='%s/GIT/AC_Agulhas_eddy_2021/Data/an64/traj_eddy1.csv' % home
data_traj_eddy1=pd.read_csv(filename_traj_eddy1, sep=',', header=0)
traj_eddy1_lon=data_traj_eddy1.values[:,0]
traj_eddy1_lat=data_traj_eddy1.values[:,1]
del data_traj_eddy1

filename_traj_eddy3='%s/GIT/AC_Agulhas_eddy_2021/Data/an64/traj_eddy3.csv' % home
data_traj_eddy3=pd.read_csv(filename_traj_eddy3, sep=',', header=0)
traj_eddy3_lon=data_traj_eddy3.values[:,0]
traj_eddy3_lat=data_traj_eddy3.values[:,1]
del data_traj_eddy3
...
a_file = open("%s/an65/data_an65.pkl" % storedir, "wb")
...
#######################################################################################################################
lon_eddy2=[8.7,8.2,7.5,6.5,6,4.5,6,6,6.8]
lat_eddy2=[33.5,33.2,34,34.2,33.8,33.5,33.7,34,35.2]
lon_eddy2=np.array(lon_eddy2)
lat_eddy2=-np.array(lat_eddy2)
...
plot4 = ax.plot(lon_eddy2, lat_eddy2, '-bx',linewidth=3.5,markersize=10,zorder=5, label='Eddy 2 trajectory')  # cmap='Blues_r')
...
plot2 = plt.scatter(0, 0, c=chl_inside_mean_4float[25],cmap='Greens', vmin=0, vmax=chl_max_plot, s=1)
a = plot2.to_rgba(chl_inside_mean_4float[25])
plt.plot(0, 0,c=a,linewidth=3, label='Eddy 1 contour')
...
for i in [10, 25, 38, 50]:
    ...
    plot2 = plt.scatter(x, y,c=lonVmax_4float[:, i]*0+chl_inside_mean_4float[i], cmap='Greens',vmin=0,vmax=chl_max_plot,s=1)
...
i=0
for i in [0,60]:
    x=lonVmax[:,i];y=latVmax[:,i]
    plot2 = plt.scatter(x, y,c=lonVmax[:, i]*0+chl_inside_mean[i], cmap='Greens',vmin=0,vmax=chl_max_plot,s=1)
    a=plot2.to_rgba(chl_inside_mean[i])
    plt.plot(x,y,c=a,linewidth=3,zorder=10)
    plt.scatter(lonEddy[i], latEddy[i], c=anom_meanE_meanOut[i], cmap='RdBu_r', vmin=-0.2, vmax=0.2,s=70,edgecolor='k',linewidth=0.5,zorder=20)
    idtmp=np.where(y==np.nanmax(y))
    x1=x[idtmp][0];y1=y[idtmp][0]
    plt.plot(np.array([lonEddy[i],x1]), np.array([latEddy[i],y1]), c='k',alpha=0.2, linewidth=2, zorder=0)
...
fig.savefig('%s/GIT/AC_Agulhas_eddy_2021/Plots/an65/Eddy_Float_Trj_and_Chl_anom_an65.pdf' % (home))
# fig.savefig('%s/GIT/AC_Agulhas_eddy_2021/Plots/an65/Eddy_Float_Trj_and_Chl_anom_larger_an65.pdf' % (home))
plt.close()
