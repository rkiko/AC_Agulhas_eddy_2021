...
sel_insideEddy = [False for i in range(Date_Num.size)]
i=0
for i in range(0,Date_Num.size):
    daytmp = datetime.strptime(Date_Time[i], '%Y-%m-%dT%H:%M:%S')
    daytmp=date.toordinal(date(daytmp.year, daytmp.month, daytmp.day))+366
...
for ipar in range(0,parameter_ylabel_list.__len__()+1):
    ...
    if ipar == 10:   color_distance = 'b'
    else:  color_distance='c'

    set_ylim_lower, set_ylim_upper = depth_filtered.min(),600
    fig = plt.figure(1, figsize=(12, 8))
    ax = fig.add_subplot(100, 1, (15, 90))
    ...
    plot2 = ax.contourf(x_filtered, y_filtered, parameter_plot)
    ...
    # I add the radius and distance
    ax2 = ax.twinx()
    ax2.set_ylim(0, 200)
    ax2.set_ylabel('distance (km)', color=color_distance)
    ax2.plot(Date_Num_Eddy_calendar, radius_Vmax, color_distance)
    ax2.scatter(list_dates, dist_km, s=20, marker='*', c=color_distance)
    ax2.tick_params(axis='y', labelcolor=color_distance)
    # I draw colorbar
    ax_clb = fig.add_subplot(40, 1, (1))
    cbar = fig.colorbar(plot2, orientation="horizontal", cax=ax_clb)
    cbar.ax.set_xlabel(parameter_ylabel_list[ipar], fontsize=18)
    plt.savefig('../Plots/an15/TimeSeries%02dA%s_an15.pdf' % (ipar,parameter_shortname_list[ipar]),dpi=200)
    plt.close()
    ...
    cbar = fig.colorbar(plot2, orientation="horizontal", cax=ax_clb)
    cbar.ax.set_xlabel(parameter_ylabel_list[ipar], fontsize=18)
    plt.savefig('../Plots/an15/TimeSeries%02dB%s_an15.pdf' % (ipar,parameter_shortname_list[ipar]),dpi=200)
    plt.close()
...
plt.grid(color='k', linestyle='dashed', linewidth=0.5)
plt.savefig('../Plots/an15/IntegratedPOC_vs_time_an15.pdf' ,dpi=200)
plt.close()
...
plt.grid(color='k', linestyle='dashed', linewidth=0.5)
plt.savefig('../Plots/an15/0_200m_MiP_MaP_bbpPOC_vs_time_an15.pdf' ,dpi=200)
plt.close()
...
plt.grid(color='k', linestyle='dashed', linewidth=0.5)
plt.savefig('../Plots/an15/200_600m_MiP_MaP_bbpPOC_vs_time_an15.pdf' ,dpi=200)
plt.close()
